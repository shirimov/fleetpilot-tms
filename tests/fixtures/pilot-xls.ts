...
export function pilotXlsFixture(options: { invoiceNumber?: string; billingDate?: string; dueDate?: string; unitNumber?: string; productCode?: string; quantity?: number; unitPrice?: number; amount?: number; total?: number; transactionDate?: string; formula?: boolean; adjustment?: number; additionalProductCode?: string; additionalAmount?: number; rowsBeforeTotal?: unknown[][] } = {}) {
  const amount = options.amount ?? 101;
  const rows: unknown[][] = [
    ['', '123456789', '', '', '', '', `InvoiceNo: ${options.invoiceNumber ?? '900001'}`, '', '', '', `Billing Date: ${options.billingDate ?? '08/25/26'} Due Date: ${options.dueDate ?? '09/01/26'}`],
    top,
    bottom,
    ['1111222233334444', options.unitNumber ?? '125', '0099', 'Dallas                  TX', 'TICKET-1', 'AUTH-1', 'Driver One', options.transactionDate ?? '08/18', 123456, options.productCode ?? '020', options.quantity ?? 10, options.unitPrice ?? 10, 100, 0, 0, 0, 0, 1, amount, 110],
  ];
  ...
  if (options.rowsBeforeTotal) rows.push(...options.rowsBeforeTotal);
