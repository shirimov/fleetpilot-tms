...
test('Administrator explores live QuickManage records, relationships and report content without credentials', async ({ page }) => {
  ...
  await page.route('**/api/integrations/quickmanage/explorer?**', async (route) => {
    ...
    if (resource === 'report-catalog') return route.fulfill({ json: { resource, fetchedAt: '2026-08-29T12:00:04Z', items: [{ type: 'trip', label: 'Trip', semantics: 'PARTIALLY_VERIFIED', sampleStatus: 'SAMPLE_AVAILABLE', reportCount: 1, countIsLowerBound: false, latestReport: { id: '33333333-3333-4333-8333-333333333333', number: 7 }, contentAvailability: 'NOT_FETCHED', firstFetchedAt: '2026-08-29T12:00:04Z', lastFetchedAt: '2026-08-29T12:00:04Z', structureFingerprint: null, importReadiness: 'DISCOVERED', blockingReason: 'Fetch and review a representative report structure.' }, { type: 'fuel', label: 'Fuel', semantics: 'UNVERIFIED', sampleStatus: 'NO_SAMPLE', reportCount: 0, countIsLowerBound: false, latestReport: null, contentAvailability: 'NOT_AVAILABLE', firstFetchedAt: '2026-08-29T12:00:04Z', lastFetchedAt: '2026-08-29T12:00:04Z', structureFingerprint: null, importReadiness: 'NOT_AVAILABLE', blockingReason: 'Generate a representative report manually in QuickManage.' }] } });
    if (resource === 'report-content') return route.fulfill({ json: { resource, fetchedAt: '2026-08-29T12:00:03Z', links: {}, item: { header: ['Trip report'], content: { columns: [{ cid: 0, key: 'amount' }], rows: [{ 0: 12.5 }] } }, audit: { reportType: 'trip', interpretation: 'PARTIALLY_VERIFIED', rowCount: 1, suppliedSummaryPresent: false, columns: [{ cid: '0', name: 'amount', description: null, systemName: null, groupName: null, dataType: null, currency: null, decimalScale: null, unit: null, aggregation: null, signSemantics: null, nullSemantics: null, zeroSemantics: null }], exactTotals: [], relationshipColumns: [], relationshipReferences: [], findings: [{ severity: 'WARNING', code: 'MONETARY_SEMANTICS_UNVERIFIED', message: 'QuickManage did not provide explicit currency/unit/precision metadata.' }] }, capture: { reportType: 'trip', subtype: null, parserVersion: 1, structureFingerprint: 'a'.repeat(64), captureStatus: 'SEMANTICS_PARTIAL', importReadiness: 'BLOCKED', blockingReasons: ['No verified idempotency identifier is available.', 'One or more field classifications require review.'], fields: [{ cid: '0', name: 'amount', declaredType: null, observedTypes: ['number'], examples: ['12.5'], classification: 'UNKNOWN', proposedClassification: 'MONEY', verification: 'UNVERIFIED', proposedBusinessMeaning: null, proposedDestination: null, moneyContract: { status: 'NOT_APPLICABLE', currency: null, representation: null, precision: null, positiveMeaning: null, negativeMeaning: null, nullMeaning: null, zeroMeaning: null } }] } } });
  ...
  await page.getByRole('button', { name: 'Scan representative samples' }).click();
  await expect(page.getByText('NO_SAMPLE')).toBeVisible();
  await expect(page.getByText('Generate a representative report manually in QuickManage.')).toBeVisible();
  await page.getByRole('button', { name: 'Fetch live data' }).click();
  await page.getByRole('button', { name: 'View details' }).click();
  ...
  await expect(page.getByRole('heading', { name: 'Report Structure Review' })).toBeVisible();
  await expect(page.getByText(`Fingerprint: ${'a'.repeat(64)}`)).toBeVisible();
  await expect(page.getByText('Suggested only: MONEY')).toBeVisible();
  await page.getByLabel('amount classification').selectOption('MONEY');
  await expect(page.getByText('Session review is not a persisted or approved import mapping.')).toBeVisible();
