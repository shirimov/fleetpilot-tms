...
export default function DriversPage() {
  ...
  const [form, setForm] = useState({ firstName: '', lastName: '', phone: '', email: '', licenseNum: '', payType: 'PERCENTAGE', payRate: '', truckId: '' })
  ...
  const submit = async (e: any) => {
    ...
    setForm({ firstName: '', lastName: '', phone: '', email: '', licenseNum: '', payType: 'PERCENTAGE', payRate: '', truckId: '' })
    setShowForm(false)
    load()
  }
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
            <form onSubmit={submit} className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-md space-y-4 max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs text-gray-400 uppercase">CDL #</label>
              ...
              <div>
                ...
                <input required type="number" step="0.01" value={form.payRate} onChange={e => setForm({ ...form, payRate: e.target.value })}
                ...
                {drivers.map((d, i) => (
                  <tr key={d.id} className={`border-b border-gray-800/50 hover:bg-gray-800/30 ${i % 2 === 0 ? '' : 'bg-gray-900/50'}`}>
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => del(d.id)} className="text-red-400 hover:text-red-300 text-xs">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
