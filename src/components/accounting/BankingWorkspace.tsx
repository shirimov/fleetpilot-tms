...
type BankTransaction = {
  ...
  suggestion: { categoryId: string; categoryName: string; scope: 'COMPANY_LEVEL' | 'ENTITY_ALLOCATED'; source: string; confidence: string; reason: string } | null;
};
type Progress = { total: number; categorized: number; uncategorized: number; reviewed: number; suggested: number; needsReview: number; unreviewed: number; ignored: number; inflowMinor: string; outflowMinor: string; categorizedInflowMinor: string; categorizedOutflowMinor: string };
type Pattern = { merchantNormalized: string; direction: string | null; count: number; totalMinor: string; earliest: string; latest: string; uncategorized: number; representativeDescriptions: string[] };
type Rule = { id: string; name: string; isEnabled: boolean; merchantNormalized: string | null; descriptionContainsNormalized: string | null; direction: string | null; category: Option; company: { name: string }; createdBy: { displayName: string | null; email: string }; createdAt: string; truck: { unitNumber: string } | null; trailer: { unitNumber: string } | null; driver: { firstName: string; lastName: string } | null; party: { name: string } | null; matchCount: number };
...
export default function BankingWorkspace() {
  ...
  const view = requestedView === 'accounts' || requestedView === 'patterns' || requestedView === 'rules' ? requestedView : 'transactions';
  ...
  const [patternSort, setPatternSort] = useState<'COUNT' | 'MONEY_OUT' | 'MONEY_IN'>('COUNT');
  ...
  const load = useCallback(async () => {
    ...
    const [nextConnections, nextTransactions, status, nextProgress, nextPatterns, nextRules] = await Promise.all([
      ...
      api<Progress>(`/api/finance/bank/categorization-progress?companyId=${encodeURIComponent(companyId)}`),
      api<Pattern[]>(`/api/finance/bank/categorization-patterns?companyId=${encodeURIComponent(companyId)}`),
      api<Rule[]>(`/api/finance/bank/categorization-rules?companyId=${encodeURIComponent(companyId)}`),
    ]);
    ...
    setProgress(nextProgress); setPatterns(nextPatterns); setRules(nextRules);
  }, [appliedCustomRange, companyId, direction, maximumAmount, minimumAmount, period, query, reviewStatus, subAccountId, uncategorized]);
  ...
  const displayedPatterns = useMemo(() => [...patterns]
    .filter((pattern) => patternSort === 'COUNT' || pattern.direction === (patternSort === 'MONEY_IN' ? 'INFLOW' : 'OUTFLOW'))
    .sort((left, right) => { if (patternSort === 'COUNT') return right.count - left.count; const leftTotal = BigInt(left.totalMinor); const rightTotal = BigInt(right.totalMinor); return rightTotal > leftTotal ? 1 : rightTotal < leftTotal ? -1 : 0; }), [patternSort, patterns]);

  async function bulkReview() {
    if (!selected.length || !bulkCategoryId) { setError('Select transactions and a category for bulk review.'); return; }
    try { await api('/api/finance/bank/transactions/bulk-review', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ transactionIds: selected, categoryId: bulkCategoryId }) }); setSelected([]); await load(); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Bulk review failed.'); }
  }
  async function createRule(event: FormEvent<HTMLFormElement>) { event.preventDefault(); try { await api(editingRuleId ? `/api/finance/bank/categorization-rules/${editingRuleId}` : '/api/finance/bank/categorization-rules', { method: editingRuleId ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ companyId, name: ruleName, merchantNormalized: ruleMerchant || null, direction: ruleDirection || null, categoryId: ruleCategoryId, scope: 'COMPANY_LEVEL' }) }); setRuleName(''); setRuleMerchant(''); setRuleDirection(''); setRuleCategoryId(''); setEditingRuleId(''); await load(); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Rule save failed.'); } }
  async function toggleRule(rule: Rule) { try { await api(`/api/finance/bank/categorization-rules/${rule.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isEnabled: !rule.isEnabled }) }); await load(); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Rule update failed.'); } }
  async function deleteRule(rule: Rule) { if (!window.confirm(`Delete rule “${rule.name}”? Transaction reviews remain unchanged.`)) return; try { await fetch(`/api/finance/bank/categorization-rules/${rule.id}`, { method: 'DELETE' }); await load(); } catch { setError('Rule deletion failed.'); } }
  async function reviewPattern(pattern: Pattern) { if (!patternCategoryId) { setError('Choose a category before reviewing a pattern.'); return; } try { await api('/api/finance/bank/transactions/bulk-review-pattern', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ companyId, merchantNormalized: pattern.merchantNormalized, direction: pattern.direction, categoryId: patternCategoryId }) }); await load(); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Pattern review failed.'); } }
  ...
  return (
    <main className="min-h-screen bg-slate-950 p-4 text-slate-100 md:p-8">
      ...
      <nav className="mb-5 flex gap-2" aria-label="Bank ledger views">
        ...
        <a className={view === 'patterns' ? 'btn' : 'rounded-lg px-3 py-2 text-sm text-slate-300'} href="/accounting/banking?view=patterns">Patterns</a>
        <a className={view === 'rules' ? 'btn' : 'rounded-lg px-3 py-2 text-sm text-slate-300'} href="/accounting/banking?view=rules">Rules</a>
      </nav>
      ...
      {view === 'accounts' ? (
        ...
        <section className="space-y-3"><h2 className="text-lg font-semibold">Normalized transaction patterns</h2><p className="text-sm text-slate-400">Conservative normalization only; ambiguous descriptions stay separate. Bulk review applies to currently reviewable matches and never creates a rule.</p><div className="flex flex-wrap gap-3"><select aria-label="Pattern ranking" className={input} value={patternSort} onChange={(event) => setPatternSort(event.target.value as typeof patternSort)}><option value="COUNT">Highest count</option><option value="MONEY_OUT">Highest MONEY OUT</option><option value="MONEY_IN">Highest MONEY IN</option></select><select aria-label="Pattern review category" className={input} value={patternCategoryId} onChange={(event) => setPatternCategoryId(event.target.value)}><option value="">Choose bulk category</option>{options?.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></div>{displayedPatterns.map((pattern) => <article className={panel} key={`${pattern.direction}:${pattern.merchantNormalized}`}><div className="flex flex-wrap justify-between gap-3"><strong>{pattern.merchantNormalized}</strong><span>{pattern.count} transactions · {money(pattern.totalMinor)}</span></div><p className="mt-1 text-xs text-slate-400">{pattern.direction ?? 'NEUTRAL'} · {pattern.uncategorized} uncategorized · {pattern.earliest.slice(0,10)}–{pattern.latest.slice(0,10)}</p><button className="mt-2 text-sm text-blue-300" onClick={() => void reviewPattern(pattern)}>Apply category to reviewable matches</button></article>)}</section>
      ) : view === 'rules' ? (
        <section className="space-y-3"><h2 className="text-lg font-semibold">Categorization rules</h2><p className="text-sm text-slate-400">Rules are visible and auditable. A match creates a suggestion only; a human must review it.</p><form className={`${panel} grid gap-3 md:grid-cols-4`} onSubmit={createRule}><input aria-label="Rule name" required className={input} value={ruleName} onChange={(event) => setRuleName(event.target.value)} placeholder="Rule name"/><input aria-label="Normalized merchant condition" className={input} value={ruleMerchant} onChange={(event) => setRuleMerchant(event.target.value)} placeholder="Merchant condition"/><select aria-label="Rule direction" className={input} value={ruleDirection} onChange={(event) => setRuleDirection(event.target.value)}><option value="">Any direction</option><option value="INFLOW">Money in</option><option value="OUTFLOW">Money out</option><option value="TRANSFER">Transfer</option></select><select aria-label="Rule category" required className={input} value={ruleCategoryId} onChange={(event) => setRuleCategoryId(event.target.value)}><option value="">Category</option>{options?.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><button className="btn">{editingRuleId ? 'Save rule changes' : 'Create suggestion rule'}</button></form>{rules.map((rule) => <article className={panel} key={rule.id}><div className="flex flex-wrap justify-between gap-3"><strong>{rule.name}</strong><div className="flex gap-2"><button className="text-sm text-blue-300" onClick={() => { setEditingRuleId(rule.id); setRuleName(rule.name); setRuleMerchant(rule.merchantNormalized ?? ''); setRuleDirection(rule.direction ?? ''); setRuleCategoryId(options?.categories.find((category) => category.name === rule.category.name)?.id ?? ''); }}>Edit</button><button className="text-sm text-blue-300" onClick={() => void toggleRule(rule)}>{rule.isEnabled ? 'Disable' : 'Enable'}</button><button className="text-sm text-red-300" onClick={() => void deleteRule(rule)}>Delete</button></div></div><p className="mt-1 text-sm text-slate-400">{rule.company.name} · {rule.merchantNormalized ?? rule.descriptionContainsNormalized ?? 'Configured condition'} · {rule.direction ?? 'Any direction'} → {rule.category.name} · {rule.truck ? `Truck ${rule.truck.unitNumber}` : rule.trailer ? `Trailer ${rule.trailer.unitNumber}` : rule.driver ? `${rule.driver.firstName} ${rule.driver.lastName}` : rule.party?.name ?? 'Company level'} · {rule.matchCount} matches</p><p className="mt-1 text-xs text-slate-500">Created by {rule.createdBy.displayName ?? rule.createdBy.email} on {new Date(rule.createdAt).toLocaleDateString()}</p></article>)}{!rules.length ? <p className={`${panel} text-sm text-slate-400`}>No categorization rules exist. Reviews do not create rules automatically.</p> : null}</section>
      ) : (
        <section className="space-y-4">
          {progress ? <div className={`${panel} grid gap-3 sm:grid-cols-3 lg:grid-cols-6`}><Metric label="Total" value={progress.total}/><Metric label="Unreviewed" value={progress.unreviewed}/><Metric label="Suggested" value={progress.suggested}/><Metric label="Needs review" value={progress.needsReview}/><Metric label="Reviewed" value={progress.reviewed}/><Metric label="Ignored" value={progress.ignored}/><Metric label="Categorized" value={progress.categorized}/><Metric label="Uncategorized" value={progress.uncategorized}/><Metric label="Money in" value={money(progress.inflowMinor)}/><Metric label="Money out" value={money(progress.outflowMinor)}/><Metric label="Categorized in" value={money(progress.categorizedInflowMinor)}/><Metric label="Categorized out" value={money(progress.categorizedOutflowMinor)}/></div> : null}
          <div className={`${panel} grid gap-3 md:grid-cols-4 lg:grid-cols-7`}>
            ...
            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={uncategorized} onChange={(event) => setUncategorized(event.target.checked)} />Uncategorized only</label>
          </div>
          {period === 'custom' ? (
          ...
          {selected.length ? <div className={`${panel} flex flex-wrap items-center gap-3`}><strong>{selected.length} selected</strong><select aria-label="Bulk review category" className={input} value={bulkCategoryId} onChange={(event) => setBulkCategoryId(event.target.value)}><option value="">Choose category</option>{options?.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><button className="btn" onClick={() => void bulkReview()}>Review selected</button><span className="text-xs text-slate-400">This does not create a future rule.</span></div> : null}
          {transactions.map((transaction) => <div key={transaction.id} className="flex gap-3"><input aria-label={`Select ${transaction.merchantName ?? transaction.originalDescription ?? 'transaction'}`} type="checkbox" checked={selected.includes(transaction.id)} onChange={(event) => setSelected((ids) => event.target.checked ? [...ids, transaction.id] : ids.filter((id) => id !== transaction.id))} /><div className="min-w-0 flex-1"><TransactionCard transaction={transaction} options={options} refresh={load} setError={setError} /></div></div>)}
...
function Metric({ label, value }: { label: string; value: string | number }) { return <div><p className="text-xs uppercase text-slate-500">{label}</p><strong>{value}</strong></div>; }
