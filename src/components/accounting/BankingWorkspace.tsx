...
type BankTransaction = {
  expectationMatches: { id: string; matchedAmountMinor: string; expectation: { id: string; reference: string | null; description: string; pilotProviderInvoice: { id: string; invoiceNumber: string } | null } }[];
  id: string;
  date: string;
...
export default function BankingWorkspace() {
  ...
  }, [appliedCustomRange, companyId, direction, maximumAmount, minimumAmount, period, query, reviewStatus, subAccountId, inbox, page]);
  useEffect(() => { setPage(1); }, [appliedCustomRange, companyId, direction, maximumAmount, minimumAmount, period, query, reviewStatus, subAccountId, inbox]);
  ...
  return (
    <main className="min-h-screen bg-slate-950 p-4 text-slate-100 md:p-8">
      ...
      <nav className="mb-5 flex gap-2" aria-label="Bank ledger views">
        ...
        <a className={view === 'transactions' ? 'btn' : 'rounded-lg px-3 py-2 text-sm text-slate-300'} href="/accounting/banking?view=transactions">Bank Transactions</a>
      </nav>
      ...
      {view === 'accounts' ? (
        ...
        <section className="space-y-4"><h2 className="text-xl font-semibold">Bank Transactions</h2><label>Bank view <select aria-label="Bank view" className={input} value={inbox ? 'review' : 'all'} onChange={event => setInbox(event.target.value === 'review')}><option value="review">Needs review</option><option value="all">All Bank Transactions / settlement history</option></select></label><p>{total} matching bank movements</p>
          ...
          <div className="flex gap-3"><button disabled={page <= 1} onClick={() => setPage(value => value - 1)}>Previous</button><span>Page {page}</span><button disabled={page * 50 >= total} onClick={() => setPage(value => value + 1)}>Next</button></div>{transactions.map((transaction) => <TransactionCard key={transaction.id} transaction={transaction} options={options} refresh={load} setError={setError} />)}
