...
export default function FuelReconciliationWorkspace({ companies, trucks }: { companies: Dimension[]; trucks: Dimension[] }) {
  ...
  return <div className="min-w-0 max-w-full space-y-4 overflow-x-hidden">
    ...
    {showPolicies ? <div className="grid gap-4 lg:grid-cols-[1fr_2fr]">
      ...
      <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-5">{[["Pilot coverage", `${result.coverage.start ?? "—"} – ${result.coverage.end ?? "—"}`],["Comparable Pilot Diesel + DEF", money(result.summary.comparablePilotMinor)],["Comparable statement fuel in coverage", money(result.summary.comparableStatementMinor)],["Raw statement fuel evidence", money(result.summary.rawFuelStatementMinor)],["All raw statement deductions", money(result.summary.rawStatementDeductionMinor)],["Expected deductions", money(result.summary.expectedMinor)],["Difference", money(result.summary.differenceMinor)],["Outside Pilot coverage", money(result.summary.outsideCoverageStatementMinor)],["Unsupported statement fuel excluded", `${result.summary.unsupportedFuelStatementCount} · ${money(result.summary.unsupportedFuelStatementMinor)}`],["Reefer excluded", money(result.summary.reeferExcludedMinor)],["Provider credit excluded", money(result.summary.providerCreditExcludedMinor)],["Historical ≠ posted", result.summary.historicalPostedDifferences]].map(([label,value]) => <article className={box} key={String(label)}><p className="text-xs text-slate-400">{label}</p><strong>{value}</strong></article>)}</div>
