...
type Row = {
  ...
  pilotEventId: string | null; pilotInvoiceId: string | null;
  ...
  statementDate: string | null; statementCardLastFour: string | null; statementLocationNumber: string | null; statementCity: string | null; statementState: string | null;
  statementGallons: string; statementRetailMinor: string | null;
  statementProducts: string[]; productClassification: "SAME" | "DIESEL_REEFER_DIFFERENCE" | "CONFLICT" | null;
  manualMatch: { id: string; reason: string; actor: string; createdAt: string } | null;
...
type Result = {
  coverage: { start: string | null; end: string | null; pilotRanges: Array<{ start: string; end: string }> };
  completeness: {
    allImported: { pilot: PilotSourceTotals; quickManage: QuickManageSourceTotals };
    comparable: { pilot: PilotSourceTotals; quickManage: QuickManageSourceTotals };
    matching: { autoMatched: number; manualMatched: number; pilotUnmatched: number; quickManageUnmatched: number; sourceCoverageGaps: number; needsReview: number; outsideComparable: number };
    conservation: {
      pilot: { countDifference: number; gallonsDifferenceHundredths: string; retailDifferenceMinor: string; dollarDifferenceMinor: string };
      quickManage: { countDifference: number; gallonsDifferenceHundredths: string; retailDifferenceMinor: string; dollarDifferenceMinor: string };
    };
    orphanRecords: number; orphanPilotRecords: number; orphanQuickManageRecords: number; orphanActiveManualMatches: number; duplicateConsumedEvidence: number;
    missingPilotCoverage: Array<{ start: string; end: string }>;
    missingQuickManageCoverage: Array<{ companyId: string | null; companyName: string | null; pid: string | null; date: string | null; reason: string | null }>;
  };
...
type PilotSourceTotals = { count: number; gallonsHundredths: string; retailMinor: string; netMinor: string; missingRetailCount: number };
type QuickManageSourceTotals = { count: number; sourceRecordCount: number; gallonsHundredths: string; retailMinor: string; deductedMinor: string; missingRetailCount: number };
...
const gallons = (hundredths: string) => `${BigInt(hundredths) / BigInt(100)}.${(BigInt(hundredths) % BigInt(100)).toString().padStart(2, "0")}`;
const statuses = ["MATCHED","UNDER_DEDUCTED","OVER_DEDUCTED","MISSING_DEDUCTION","STATEMENT_ONLY","PILOT_UNMATCHED","QM_UNMATCHED","SOURCE_COVERAGE_GAP","TIMING_DIFFERENCE","NO_PILOT_DATA_IMPORTED","NEEDS_COMPANY_HISTORY","NEEDS_TRUCK_MAPPING","NEEDS_RECIPIENT_MAPPING","NEEDS_RECIPIENT_REVIEW","PRODUCT_CLASSIFICATION_REVIEW","NEEDS_POLICY","NEEDS_REVIEW"];
const matchExplanations: Record<string, string> = {
  ...
  MANUAL_OWNER_MATCH: "An OWNER paired these immutable Pilot and QuickManage source records. Policy logic evaluates the amount separately.",
  MISSING_QUICKMANAGE_COVERAGE: "QuickManage source coverage is unavailable for this Company/date, so no financial Missing conclusion is made.",
  MISSING_PILOT_COVERAGE: "Pilot source coverage is unavailable for this QuickManage line/date, so no financial Over conclusion is made.",
  NO_DEDUCTION_EXPECTED: "The effective policy assigns fuel responsibility to the Company, so no QuickManage recipient deduction is expected.",
};
...
function SourceSide({ title, row }: { title: "PILOT" | "QUICKMANAGE"; row: Row | null }) {
  if (!row) return <article className="rounded border border-dashed border-white/20 p-3 text-sm text-slate-400">Select a {title === "PILOT" ? "Pilot unmatched" : "QM unmatched"} row below.</article>;
  const pilot = title === "PILOT";
  return <article className="rounded border border-white/10 p-3 text-xs"><h4 className="font-semibold">{title}</h4><p>Truck {pilot ? row.truckUnit ?? "—" : row.statementTruckUnit ?? "—"} · {row.companyName ?? "unresolved Company"}</p><p>{pilot ? row.recipientName ?? "unresolved recipient" : row.statementRecipientName ?? "unresolved recipient"}</p><p>{pilot ? row.purchaseDate ?? "—" : row.statementDate ?? row.statementPeriod ?? "—"} · card {pilot ? row.pilotCardLastFour ?? "—" : row.statementCardLastFour ?? "—"}</p><p>Location {pilot ? row.pilotLocationNumber ?? "—" : row.statementLocationNumber ?? "—"} · {pilot ? row.pilotCity ?? "—" : row.statementCity ?? "—"}, {pilot ? row.pilotState ?? "—" : row.statementState ?? "—"}</p><p>{pilot ? row.gallons : row.statementGallons} gal · {(pilot ? row.products : row.statementProducts).join(", ") || "—"}</p><p>Retail {money(pilot ? row.pilotRetailMinor : row.statementRetailMinor)} · {pilot ? `net ${money(row.pilotActualMinor)}` : `deducted ${money(row.statementMinor)}`}</p><p>Expected {money(row.expectedMinor)} · difference {money(row.differenceMinor)}</p></article>;
}
...
export default function FuelReconciliationWorkspace({ companies, trucks }: { companies: Dimension[]; trucks: Dimension[] }) {
  ...
  const [selectedPilot, setSelectedPilot] = useState<Row | null>(null), [selectedQuickManage, setSelectedQuickManage] = useState<Row | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([request<Result>(`/api/finance/fuel-reconciliation?${new URLSearchParams([...new URLSearchParams(query)].filter(([key]) => ["page","company","pid","date","truck","recipient","responsibility","status","policy","history","match","queue"].includes(key)))}`, { signal: controller.signal }), request<Policy[]>("/api/finance/fuel-reconciliation?view=policies", { signal: controller.signal })])
  ...
  const refreshPreview = async () => setResult(await request<Result>(`/api/finance/fuel-reconciliation?${new URLSearchParams([...new URLSearchParams(query)].filter(([key]) => ["page","company","pid","date","truck","recipient","responsibility","status","policy","history","match","queue"].includes(key)))}`));
  async function saveManualMatch(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!selectedPilot?.pilotEventId || !selectedQuickManage?.statementEvidence?.groupLineId) return;
    setBusy(true); setError(""); const form = event.currentTarget;
    try {
      await request("/api/finance/fuel-reconciliation", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "MANUAL_MATCH", pilotEventId: selectedPilot.pilotEventId, archiveLineId: selectedQuickManage.statementEvidence.groupLineId, reason: String(new FormData(form).get("reason") ?? "") }) });
      setSelectedPilot(null); setSelectedQuickManage(null); await refreshPreview();
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Manual match could not be saved."); } finally { setBusy(false); }
  }
  async function unmatchManual(event: React.FormEvent<HTMLFormElement>, matchId: string) {
    event.preventDefault(); setBusy(true); setError(""); const form = event.currentTarget;
    try {
      await request("/api/finance/fuel-reconciliation", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "MANUAL_UNMATCH", matchId, reason: String(new FormData(form).get("reason") ?? "") }) });
      await refreshPreview();
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Manual match could not be undone."); } finally { setBusy(false); }
  }
  ...
  return <div className="min-w-0 max-w-full space-y-4 overflow-x-hidden">
    ...
    {showPolicies ? <div className="grid gap-4 lg:grid-cols-[1fr_2fr]">
      ...
      <section className={`${box} space-y-3`} aria-label="Fuel source completeness">
        <div><h2 className="font-semibold">FUEL SOURCE COMPLETENESS</h2><p className="text-xs text-slate-400">Every imported Pilot fuel transaction and normalized QuickManage fuel deduction is assigned exactly once. Matching identity and deduction accuracy remain separate controls.</p></div>
        <div className="grid gap-3 md:grid-cols-2">
          <a className={`${box} block hover:border-emerald-400/60`} href="#fuel-reconciliation-table"><h3 className="font-semibold">ALL IMPORTED DATA · PILOT</h3><p>{result.completeness.allImported.pilot.count} transactions · {gallons(result.completeness.allImported.pilot.gallonsHundredths)} gal</p><p>Retail {money(result.completeness.allImported.pilot.retailMinor)} · net {money(result.completeness.allImported.pilot.netMinor)}</p></a>
          <a className={`${box} block hover:border-emerald-400/60`} href="#fuel-reconciliation-table"><h3 className="font-semibold">ALL IMPORTED DATA · QUICKMANAGE</h3><p>{result.completeness.allImported.quickManage.count} transactions · {result.completeness.allImported.quickManage.sourceRecordCount} source lines · {gallons(result.completeness.allImported.quickManage.gallonsHundredths)} gal</p><p>Retail {money(result.completeness.allImported.quickManage.retailMinor)} · deducted {money(result.completeness.allImported.quickManage.deductedMinor)}</p></a>
          <a className={`${box} block hover:border-emerald-400/60`} href="#fuel-reconciliation-table"><h3 className="font-semibold">COMPARABLE COVERAGE · PILOT</h3><p>{result.completeness.comparable.pilot.count} transactions · {gallons(result.completeness.comparable.pilot.gallonsHundredths)} gal</p><p>Retail {money(result.completeness.comparable.pilot.retailMinor)} · net {money(result.completeness.comparable.pilot.netMinor)}</p></a>
          <a className={`${box} block hover:border-emerald-400/60`} href="#fuel-reconciliation-table"><h3 className="font-semibold">COMPARABLE COVERAGE · QUICKMANAGE</h3><p>{result.completeness.comparable.quickManage.count} transactions · {gallons(result.completeness.comparable.quickManage.gallonsHundredths)} gal</p><p>Retail {money(result.completeness.comparable.quickManage.retailMinor)} · deducted {money(result.completeness.comparable.quickManage.deductedMinor)}</p></a>
        </div>
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{[
          ["Auto matched", result.completeness.matching.autoMatched, "match=auto"], ["Manual matched", result.completeness.matching.manualMatched, "match=manual"],
          ["Pilot unmatched", result.completeness.matching.pilotUnmatched, "status=PILOT_UNMATCHED"], ["QM unmatched", result.completeness.matching.quickManageUnmatched, "status=QM_UNMATCHED"],
          ["Coverage gaps", result.completeness.matching.sourceCoverageGaps, "status=SOURCE_COVERAGE_GAP"], ["Needs review", result.completeness.matching.needsReview, "queue=review"],
          ["Outside comparable", result.completeness.matching.outsideComparable, ""], ["Orphan records", result.completeness.orphanRecords, ""], ["Duplicate consumption", result.completeness.duplicateConsumedEvidence, ""],
        ].map(([label, value, filter]) => <a className="rounded border border-white/10 bg-slate-950/60 p-3 hover:border-emerald-400/60" href={filter ? `/accounting?view=statements&archive=reconciliation&${filter}` : "#fuel-reconciliation-table"} key={String(label)}><span className="text-xs text-slate-400">{label}</span><strong className="block text-xl">{value}</strong></a>)}</div>
        <div className="grid gap-2 text-xs md:grid-cols-2"><a className="rounded border border-white/10 p-2 hover:border-emerald-400/60" href="#fuel-reconciliation-table"><strong>Pilot conservation:</strong> count {result.completeness.conservation.pilot.countDifference} · gallons {gallons(result.completeness.conservation.pilot.gallonsDifferenceHundredths)} · retail {money(result.completeness.conservation.pilot.retailDifferenceMinor)} · net {money(result.completeness.conservation.pilot.dollarDifferenceMinor)}</a><a className="rounded border border-white/10 p-2 hover:border-emerald-400/60" href="#fuel-reconciliation-table"><strong>QM conservation:</strong> count {result.completeness.conservation.quickManage.countDifference} · gallons {gallons(result.completeness.conservation.quickManage.gallonsDifferenceHundredths)} · retail {money(result.completeness.conservation.quickManage.retailDifferenceMinor)} · deducted {money(result.completeness.conservation.quickManage.dollarDifferenceMinor)}</a></div>
        <details className="text-xs"><summary className="cursor-pointer text-emerald-300">Source coverage gaps</summary><div className="mt-2 grid gap-2 md:grid-cols-2"><div><strong>Missing Pilot periods</strong>{result.completeness.missingPilotCoverage.map(range => <p key={`${range.start}-${range.end}`}>{range.start}–{range.end}</p>)}{!result.completeness.missingPilotCoverage.length && <p>None detected between imported periods.</p>}</div><div><strong>Missing QuickManage Company/date coverage</strong>{result.completeness.missingQuickManageCoverage.map((gap, index) => <p key={`${gap.companyId}-${gap.date}-${index}`}>{gap.companyName ?? "Unresolved Company"} · {gap.pid ? `PID ${gap.pid}` : "PID unavailable"} · {gap.date ?? "unknown date"}</p>)}{!result.completeness.missingQuickManageCoverage.length && <p>None detected for imported Pilot transactions.</p>}</div></div></details>
      </section>
      {(selectedPilot || selectedQuickManage) && <form className={`${box} space-y-3`} onSubmit={saveManualMatch}><div className="flex items-center justify-between"><h3 className="font-semibold">MANUAL MATCH</h3><button type="button" onClick={() => { setSelectedPilot(null); setSelectedQuickManage(null); }}>Clear</button></div><div className="grid gap-3 md:grid-cols-2"><SourceSide title="PILOT" row={selectedPilot} /><SourceSide title="QUICKMANAGE" row={selectedQuickManage} /></div><label className="grid gap-1 text-xs">OWNER reason<textarea className={input} name="reason" minLength={10} maxLength={2000} required /></label><label className="flex items-start gap-2 text-xs"><input name="confirmed" required type="checkbox" />I confirm these are the same fuel purchase. Source evidence will remain unchanged.</label><button className="btn" disabled={busy || !selectedPilot || !selectedQuickManage}>{busy ? "Saving…" : "Save audited manual match"}</button></form>}
