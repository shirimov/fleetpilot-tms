...
export default function PilotImportWorkspace({ sources, categories, trucks, mode = 'imports', selectedId }: Props) {
  ...
  async function resolve(issue: Row, action: string, reference?: string, historicalInactiveAttribution = false) {
    if (!invoice) return;
    const body = action === 'MATCH_TRUCK' ? { action, truckId: reference, historicalInactiveAttribution } : action === 'SET_CATEGORY' ? { action, categoryId: reference } : { action };
    setBusy(true); setError('');
...
function Issue({ issue, busy, trucks, categories, resolve }: { issue: Row; busy: boolean; trucks: Row[]; categories: Row[]; resolve: (issue: Row, action: string, reference?: string, historicalInactiveAttribution?: boolean) => Promise<void> }) {
  ...
  const selectedTruck = trucks.find((truck) => String(truck.id) === selectedTruckId);
  const selectedInactive = selectedTruck?.status === 'INACTIVE';
  return <div className="rounded-lg bg-slate-950/60 p-3"><div className="flex justify-between gap-3"><div><strong className={issue.status === 'OPEN' ? 'text-amber-200' : 'text-emerald-300'}>{String(issue.code).replaceAll('_', ' ')}</strong><p className="text-xs text-slate-400">{String(issue.message)}</p></div><span className="text-xs">{String(issue.status)}</span></div>{issue.status === 'OPEN' && <div className="mt-2">{truckIssue && <div className="grid gap-2 sm:grid-cols-2"><input aria-label={`Search trucks for ${String(issue.code)}`} className={field} value={truckSearch} onChange={(event) => setTruckSearch(event.target.value)} placeholder="Search unit or company" /><select aria-label={`Resolve ${String(issue.code)}`} className={field} value={selectedTruckId} disabled={busy} onChange={(event) => { const truck = trucks.find((row) => String(row.id) === event.target.value); setSelectedTruckId(event.target.value); if (truck && truck.status !== 'INACTIVE') void resolve(issue, 'MATCH_TRUCK', event.target.value); }}><option value="">Match exact truck…</option>{shownTrucks.map((truck) => <option key={String(truck.id)} value={String(truck.id)}>Truck {String(truck.unitNumber)} — {String((truck.company as Row)?.name ?? '')} — {String(truck.status)}</option>)}</select>{selectedInactive && <div className="sm:col-span-2 rounded-lg border border-amber-500/40 bg-amber-950/40 p-3"><p className="text-sm font-semibold text-amber-200">Inactive Truck — historical attribution review required</p><p className="mt-1 text-xs text-amber-100">This Truck is currently inactive. Selecting it attributes this historical Pilot transaction to the canonical Truck; it does not reactivate the Truck.</p><button type="button" className="btn mt-2" disabled={busy} onClick={() => resolve(issue, 'MATCH_TRUCK', selectedTruckId, true)}>Confirm historical attribution</button></div>}</div>}{categoryIssue && <select aria-label={`Resolve ${String(issue.code)}`} className={field} defaultValue="" disabled={busy} onChange={(event) => event.target.value && resolve(issue, 'SET_CATEGORY', event.target.value)}><option value="">Assign accounting category…</option>{selectableCategories.map((category) => <option key={String(category.id)} value={String(category.id)}>{String(category.path ?? category.name)}</option>)}</select>}{issue.code === 'OUTSIDE_PERIOD' && <button className="btn" disabled={busy} onClick={() => resolve(issue, 'ACKNOWLEDGE')}>Acknowledge reviewed date</button>}</div>}</div>;
}
