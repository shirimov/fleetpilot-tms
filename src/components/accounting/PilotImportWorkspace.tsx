  ...
  return <div className="space-y-4">
    ...
    {invoice && <>
      <section className={card}>
        <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs uppercase text-emerald-400">Pilot invoice</p><h2 className="text-xl font-semibold">{String(invoice.invoiceNumber)}</h2><p className="text-xs text-slate-400">Period {String(invoice.periodStart).slice(0, 10)} – {String(invoice.periodEnd).slice(0, 10)} · Due {invoice.dueDate ? String(invoice.dueDate).slice(0, 10) : 'not provided'} · Parser {String(invoice.parseVersion)}</p></div><div className="flex flex-wrap gap-2">{invoice.canReparse === true && <button className="btn" disabled={busy} onClick={reparse}>Reparse invoice</button>}<button className="btn" disabled={busy || invoice.status !== 'READY_TO_POST'} onClick={post}>Post reconciled invoice</button></div></div>
