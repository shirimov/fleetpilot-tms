...
export default function QuickManageExplorer() {
  ...
  return (
    <section className="mt-8 border-t border-gray-800 pt-8" aria-labelledby="quickmanage-explorer-heading">
      ...
      {selected && <div className="mt-5 rounded-lg border border-blue-900 bg-gray-950 p-4"><div className="flex justify-between"><h3 className="font-semibold">Live QuickManage detail</h3><button type="button" onClick={() => setSelected(null)} className="text-sm text-gray-400">Close</button></div>{reverseTripFilter[resource] && typeof selected.id === 'string' && <button type="button" onClick={() => void load(0, 'trips', { field: reverseTripFilter[resource]!, value: String(selected.id) })} className="mt-3 rounded border border-blue-800 px-2 py-1 text-xs text-blue-300">Find related Trips</button>}{resource === 'trips' && tripRelationships(selected).length > 0 && <div className="mt-3 flex flex-wrap gap-2"><span className="text-sm text-gray-400">Related:</span>{tripRelationships(selected).map((entry) => <button key={`${entry.resource}:${entry.id}`} type="button" onClick={() => void load(0, entry.resource, { field: entry.field, value: entry.id })} className="rounded border border-blue-800 px-2 py-1 text-xs text-blue-300">{entry.label}</button>)}</div>}{resource === 'reports' && selected.content != null && typeof selected.content === 'object' && !Array.isArray(selected.content) && Array.isArray((selected.content as { rows?: unknown }).rows) && <div className="mt-4"><h4 className="text-sm font-semibold">Report line items</h4><div className="mt-2 max-h-80 overflow-auto rounded border border-gray-800">{((selected.content as { rows: unknown[] }).rows).map((row, index) => <pre key={index} className="border-b border-gray-800 p-3 text-xs text-gray-300">{JSON.stringify(row, null, 2)}</pre>)}</div></div>}<details className="mt-3"><summary className="cursor-pointer text-sm font-semibold text-blue-300">Raw QuickManage Data</summary><pre className="mt-3 max-h-[32rem] overflow-auto whitespace-pre-wrap break-all rounded bg-black p-3 text-xs text-gray-300">{JSON.stringify(selected, null, 2)}</pre></details></div>}
    </section>
  );
}
