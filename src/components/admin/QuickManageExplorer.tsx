...
type CatalogItem = {
  type: string; label: string; semantics: string; sampleStatus: 'NO_SAMPLE' | 'SAMPLE_AVAILABLE'; reportCount: number;
  countIsLowerBound: boolean; latestReport: Record<string, unknown> | null; contentAvailability: string;
  firstFetchedAt: string; lastFetchedAt: string; structureFingerprint: string | null; importReadiness: string; blockingReason: string;
};
type ReportCapture = {
  reportType: string; subtype: string | null; parserVersion: number; structureFingerprint: string; captureStatus: string;
  importReadiness: string; blockingReasons: string[];
  fields: Array<{ cid: string; name: string; declaredType: string | null; observedTypes: string[]; examples: string[]; classification: string; proposedClassification: string | null; verification: string; proposedBusinessMeaning: string | null; proposedDestination: string | null; moneyContract: { status: string; currency: string | null; representation: string | null; precision: number | null; positiveMeaning: string | null; negativeMeaning: string | null; nullMeaning: string | null; zeroMeaning: string | null } }>;
};
type FinancialAudit = {
  reportType: string;
  interpretation: 'PARTIALLY_VERIFIED' | 'UNVERIFIED';
  columns: Array<{ cid: string; name: string; description: string | null; systemName: string | null; groupName: string | null; dataType: string | null; currency: string | null; decimalScale: number | null; unit: string | null; aggregation: string | null; signSemantics: string | null; nullSemantics: string | null; zeroSemantics: string | null }>;
  rowCount: number;
...
const fieldClassifications = ['IDENTIFIER', 'DATE', 'DATETIME', 'TEXT', 'STATUS', 'RELATIONSHIP_ID', 'MONEY', 'QUANTITY', 'RATE', 'PERCENT', 'BOOLEAN', 'UNKNOWN'];
const accountingConcepts = ['LOAD_REVENUE', 'LINEHAUL', 'FUEL_SURCHARGE', 'ACCESSORIAL', 'DETENTION', 'LUMPER', 'DRIVER_PAY', 'OWNER_PAY', 'FUEL_EXPENSE', 'TOLL_EXPENSE', 'MAINTENANCE_EXPENSE', 'ADVANCE', 'DEDUCTION', 'REIMBURSEMENT', 'ADJUSTMENT', 'RECEIVABLE', 'PAYMENT', 'INVOICE_AMOUNT', 'BALANCE', 'OTHER'];
...
export default function QuickManageExplorer() {
  ...
  const [capture, setCapture] = useState<ReportCapture | null>(null);
  const [previousCaptures, setPreviousCaptures] = useState<Record<string, ReportCapture>>({});
  const [structureChange, setStructureChange] = useState<string | null>(null);
  ...
  async function load(page = 0, explicitResource: Resource = resource, relationship?: { field: string; value: string }) {
    setLoading(true); setError(''); setSelected(null); setSelectedReport(null); setAudit(null); setCapture(null); setStructureChange(null);
    try {
  ...
  async function openReport(record: Record<string, unknown>) {
    ...
    try {
      ...
      const nextCapture = body.capture as ReportCapture;
      const prior = previousCaptures[nextCapture.reportType];
      setStructureChange(prior ? prior.structureFingerprint === nextCapture.structureFingerprint ? 'Structure matches the previous sample fetched in this session.' : 'STRUCTURE CHANGED — review mappings before reuse.' : null);
      setPreviousCaptures((current) => ({ ...current, [nextCapture.reportType]: nextCapture }));
  ...
  async function loadCatalog() {
    ...
    try {
      const response = await fetch('/api/integrations/quickmanage/explorer?resource=report-catalog', { cache: 'no-store' });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? 'Report sample scan failed.');
      setCatalog((current) => (body.items as CatalogItem[]).map((entry) => ({
        ...entry,
        firstFetchedAt: current?.find((previous) => previous.type === entry.type)?.firstFetchedAt ?? entry.firstFetchedAt,
        lastFetchedAt: entry.lastFetchedAt,
      })));
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Report sample scan failed.'); }
    finally { setCatalogLoading(false); }
  }

  function reviewField(fieldName: string, property: 'classification' | 'proposedDestination', value: string) {
    setCapture((current) => current ? {
      ...current,
      captureStatus: 'STRUCTURE_REVIEWED',
      importReadiness: 'BLOCKED',
      fields: current.fields.map((field) => field.name === fieldName ? { ...field, [property]: value || null, verification: 'ADMIN_VERIFIED', proposedClassification: null } : field),
      blockingReasons: [...new Set([...current.blockingReasons, 'Session review is not a persisted or approved import mapping.'])],
    } : current);
  }

  function chooseResource(next: Resource) {
    setResource(next); setFilterField(filters[next][0]?.field ?? ''); setFilterValue(''); setResult(null); setSelected(null); setSelectedReport(null); setAudit(null); setCapture(null); setStructureChange(null); setError('');
  }
  ...
  return (
    <section className="mt-8 border-t border-gray-800 pt-8" aria-labelledby="quickmanage-explorer-heading">
      ...
      {resource === 'reports' && <div className="mt-4 rounded-lg border border-blue-950 bg-blue-950/20 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">Financial Reports / Audit</h3><p className="mt-1 text-sm text-gray-400">Previously generated QuickManage reports, fetched on demand. Vendor semantics stay marked unverified until official metadata or representative data proves them.</p></div><button type="button" onClick={() => void loadCatalog()} disabled={catalogLoading} className="rounded border border-blue-700 px-3 py-2 text-sm font-semibold text-blue-200 disabled:opacity-50">{catalogLoading ? 'Scanning…' : 'Scan representative samples'}</button></div></div>}
      {resource === 'reports' && catalog && <div className="mt-4 overflow-auto rounded-lg border border-gray-800"><table className="w-full min-w-[72rem] text-left text-xs"><thead className="bg-gray-950 text-gray-400"><tr><th className="p-3">Report type</th><th className="p-3">Samples</th><th className="p-3">Latest report</th><th className="p-3">Current exploration</th><th className="p-3">Structure</th><th className="p-3">Semantics</th><th className="p-3">Import readiness</th><th className="p-3">Blocking reason</th></tr></thead><tbody>{catalog.map((entry) => <tr key={entry.type} className="border-t border-gray-800"><td className="p-3 font-semibold">{entry.label}<div className="font-mono text-[10px] text-gray-500">{entry.type}</div></td><td className="p-3">{entry.sampleStatus}<div className="text-gray-500">{entry.countIsLowerBound ? `${entry.reportCount}+` : entry.reportCount} on catalog</div></td><td className="p-3">{entry.latestReport ? display(entry.latestReport.number ?? entry.latestReport.id) : '—'}</td><td className="p-3 text-gray-500">First: {new Date(entry.firstFetchedAt).toLocaleTimeString()}<br />Last: {new Date(entry.lastFetchedAt).toLocaleTimeString()}</td><td className="p-3">{previousCaptures[entry.type]?.structureFingerprint.slice(0, 12) ?? 'Not fetched'}</td><td className="p-3">{previousCaptures[entry.type]?.captureStatus ?? entry.semantics}</td><td className="p-3">{previousCaptures[entry.type]?.importReadiness ?? entry.importReadiness}</td><td className="p-3 text-gray-400">{previousCaptures[entry.type]?.blockingReasons.join(' ') ?? entry.blockingReason}</td></tr>)}</tbody></table></div>}
      ...
      {selected && <div className="mt-5 rounded-lg border border-blue-900 bg-gray-950 p-4"><div className="flex justify-between"><h3 className="font-semibold">{resource === 'reports' ? 'QuickManage financial report detail' : 'Live QuickManage detail'}</h3><button type="button" onClick={() => { setSelected(null); setSelectedReport(null); setAudit(null); setCapture(null); setStructureChange(null); }} className="text-sm text-gray-400">Close</button></div>{resource === 'reports' && selectedReport && <dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2">{['id', 'type', 'subtype', 'number', 'start_date', 'end_date', 'created_at', 'updated_at'].map((key) => selectedReport[key] == null ? null : <div key={key}><dt className="text-gray-500">{key}</dt><dd>{display(selectedReport[key])}</dd></div>)}</dl>}{reverseTripFilter[resource] && typeof selected.id === 'string' && <button type="button" onClick={() => void load(0, 'trips', { field: reverseTripFilter[resource]!, value: String(selected.id) })} className="mt-3 rounded border border-blue-800 px-2 py-1 text-xs text-blue-300">Find related Trips</button>}{resource === 'trips' && tripRelationships(selected).length > 0 && <div className="mt-3 flex flex-wrap gap-2"><span className="text-sm text-gray-400">Related:</span>{tripRelationships(selected).map((entry) => <button key={`${entry.resource}:${entry.id}`} type="button" onClick={() => void load(0, entry.resource, { field: entry.field, value: entry.id })} className="rounded border border-blue-800 px-2 py-1 text-xs text-blue-300">{entry.label}</button>)}</div>}{resource === 'reports' && audit && <div className="mt-4 space-y-4"><div className="rounded border border-amber-900 bg-amber-950/20 p-3"><p className="font-semibold">{audit.interpretation === 'UNVERIFIED' ? 'QuickManage semantics not yet verified' : 'QuickManage semantics partially verified'}</p><p className="mt-1 text-sm text-gray-400">{audit.rowCount} rows · {audit.columns.length} dynamic columns · supplied structured summary: {audit.suppliedSummaryPresent ? 'yes' : 'no'}</p></div>{capture && <div className="rounded border border-gray-800 p-3"><h4 className="text-sm font-semibold">Report Structure Review</h4><p className="mt-1 break-all font-mono text-xs text-gray-400">Fingerprint: {capture.structureFingerprint}</p><p className="mt-1 text-sm">Capture: {capture.captureStatus} · Import readiness: {capture.importReadiness}</p>{structureChange && <p className={`mt-2 text-sm ${structureChange.startsWith('STRUCTURE CHANGED') ? 'text-red-300' : 'text-green-300'}`}>{structureChange}</p>}<p className="mt-2 text-xs text-amber-300">Review choices are session-only and never create or enable an accounting import.</p><div className="mt-3 overflow-auto"><table className="w-full min-w-[68rem] text-left text-xs"><thead className="text-gray-500"><tr><th className="p-2">QuickManage field</th><th className="p-2">Examples</th><th className="p-2">Observed</th><th className="p-2">Classification</th><th className="p-2">Verification</th><th className="p-2">FleetPilot concept</th><th className="p-2">Money contract</th></tr></thead><tbody>{capture.fields.map((field) => <tr key={field.cid} className="border-t border-gray-800"><td className="p-2">{field.name}</td><td className="p-2">{field.examples.join(' · ') || '—'}</td><td className="p-2">{field.observedTypes.join(', ') || '—'}</td><td className="p-2"><select aria-label={`${field.name} classification`} value={field.classification} onChange={(event) => reviewField(field.name, 'classification', event.target.value)} className="rounded border border-gray-700 bg-gray-900 p-1">{fieldClassifications.map((classification) => <option key={classification}>{classification}</option>)}</select>{field.proposedClassification && <div className="mt-1 text-amber-300">Suggested only: {field.proposedClassification}</div>}</td><td className="p-2">{field.verification}</td><td className="p-2"><select aria-label={`${field.name} FleetPilot concept`} value={field.proposedDestination ?? ''} onChange={(event) => reviewField(field.name, 'proposedDestination', event.target.value)} className="rounded border border-gray-700 bg-gray-900 p-1"><option value="">Not mapped</option>{accountingConcepts.map((concept) => <option key={concept}>{concept}</option>)}</select></td><td className="p-2">{field.moneyContract.status}</td></tr>)}</tbody></table></div><ul className="mt-3 list-disc space-y-1 pl-5 text-xs text-amber-200">{capture.blockingReasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></div>}<div><h4 className="text-sm font-semibold">Column definitions</h4><div className="mt-2 overflow-auto"><table className="w-full min-w-[48rem] text-left text-xs"><thead className="text-gray-500"><tr><th className="p-2">Column</th><th className="p-2">Provider field</th><th className="p-2">Type / unit / scale</th><th className="p-2">Sign</th></tr></thead><tbody>{audit.columns.map((column) => <tr key={column.cid} className="border-t border-gray-800"><td className="p-2">{column.name}</td><td className="p-2">{column.systemName ?? '—'}</td><td className="p-2">{[column.dataType, column.unit, column.decimalScale == null ? null : `scale ${column.decimalScale}`, column.aggregation].filter(Boolean).join(' · ') || 'Unspecified'}</td><td className="p-2">{column.signSemantics ?? 'Unknown'}</td></tr>)}</tbody></table></div></div>{audit.relationshipReferences.length > 0 && <div><h4 className="text-sm font-semibold">Verified ID relationships</h4><ul className="mt-2 space-y-1 text-sm">{audit.relationshipReferences.map((reference) => <li key={`${reference.resource}:${reference.externalId}`}>{reference.resource} · {reference.externalId} · {result?.links?.[`${reference.resource}:${reference.externalId}`] ? 'linked to FleetPilot' : 'not linked to FleetPilot'}</li>)}</ul></div>}<div><h4 className="text-sm font-semibold">Financial totals</h4>{audit.exactTotals.length === 0 ? <p className="mt-1 text-sm text-gray-400">No safe total calculated. Currency, precision, and sum semantics must be explicit.</p> : <div className="mt-2 space-y-2">{audit.exactTotals.map((total) => <div key={total.column} className="rounded border border-gray-800 p-3 text-sm"><p>{total.column} · {total.currency ?? 'currency unspecified'}</p><p>FleetPilot calculated total: {total.calculated}</p><p>QuickManage supplied total: {total.supplied ?? 'not supplied'}</p></div>)}</div>}</div><div><h4 className="text-sm font-semibold">Audit findings</h4><ul className="mt-2 space-y-2">{audit.findings.map((finding, index) => <li key={`${finding.code}:${index}`} className="rounded border border-gray-800 p-3 text-sm"><span className={finding.severity === 'ERROR' ? 'text-red-300' : finding.severity === 'WARNING' ? 'text-amber-300' : 'text-blue-300'}>{finding.severity}</span> · {finding.message}</li>)}</ul></div></div>}{resource === 'reports' && selected.content != null && typeof selected.content === 'object' && !Array.isArray(selected.content) && Array.isArray((selected.content as { rows?: unknown }).rows) && <div className="mt-4"><h4 className="text-sm font-semibold">Report line items</h4><div className="mt-2 max-h-80 overflow-auto rounded border border-gray-800">{((selected.content as { rows: unknown[] }).rows).map((row, index) => <pre key={index} className="border-b border-gray-800 p-3 text-xs text-gray-300">{JSON.stringify(row, null, 2)}</pre>)}</div></div>}<details className="mt-3"><summary className="cursor-pointer text-sm font-semibold text-blue-300">Sanitized raw QuickManage data</summary><pre className="mt-3 max-h-[32rem] overflow-auto whitespace-pre-wrap break-all rounded bg-black p-3 text-xs text-gray-300">{JSON.stringify(selected, null, 2)}</pre></details></div>}
    </section>
  );
}
