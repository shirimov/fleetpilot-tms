...
test("fixed-pay ordering retries and concurrency preserve raw evidence; changed values/PDF conflict; newer version remains distinct", async () => {
  ...
  f.payload.data.fixed_pays[0].amount += 0.01;
  const changed = {
    ...f.bundle,
    detail: Buffer.from(JSON.stringify(f.payload)),
  };
