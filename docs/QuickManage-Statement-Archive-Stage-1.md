...
## Final integrity review amendments

The review adds `20260919070000_archive_integrity_guards` without changing existing financial rows. Coverage compares provider timestamps as UTC instants even when the PostgreSQL session uses another timezone. Child insertion fails closed when its parent is not visible, closing a concurrency gap where foreign-key validation could otherwise wait for an already-sealed parent to commit. Both cases have database regressions.

Inventory UUIDs are canonicalized to lowercase after validation; case variants cannot create separate identities or prevent a match to a normalized captured UUID. Original source metadata and fingerprints still preserve the received representation. Conflicting exact VIN and unit matches leave Truck mapping unresolved while retaining the statement. List filters reject invalid PID, lifecycle, type, status, and oversized text before querying.

The expanded synthetic suite captures 9,127 statements through the actual archive service across 14 Companies and 310 Company/PID groups, including 8,276 terminated recipients and 92 additional provider versions. It checks exact overview totals, Company isolation, pagination, bounded Prisma operation counts, and zero transactions, allocations, expectations, bank matches, or Pilot events. A separate 42-item interruption/resume test verifies one PDF, JSON version, line set, and success audit per identity, repeated resume after completion, immutable older snapshots, equal-count identity substitution, and excess captures. Failure tests preserve unrelated successes through PDF, detail, parse, timeout, and authentication failures.

For the future 5–10 unique-statement live sample, explicitly include an **active Driver**, **active Contractor**, **terminated Driver/Contractor**, a **fuel deduction**, and **multiple deductions**; cases may overlap. For each selected UUID record inventory identity, PDF checksum, raw JSON/version checksum, normalized lines, canonical or unresolved mapping, idempotent second capture, and exact completeness impact. This is a plan only: live credentials remain unverified, capture remains disabled, and no live acceptance or backfill is authorized by this review.
