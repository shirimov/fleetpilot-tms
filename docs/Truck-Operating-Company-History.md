...
- Deferred constraint triggers require one open interval matching `Truck.companyId` after every tracked Truck/timeline mutation; initial revisions cannot commit without a valid timeline. Affiliation parent Truck and revision must agree, including for an otherwise untracked Truck. Revision predecessors must belong to the same Truck, and active periods cannot be attached to a superseded revision.
...
## Final review and batch follow-up
...
The resolver is suitable for individual detail requests. It currently loads authorization and history per call; it does not offer a batch API. Before processing fleet-wide Pilot events or statements, add a scoped batch resolver that loads memberships and relevant periods once and resolves ranges in memory. This is required for future bulk reconciliation, not for this detail-only foundation. Existing Pilot posting has not been switched to this resolver.
